...
void main() {
  ...
  // vanilla bedrock bug: chunks near the edge of render distance can lose
  // proper light data and render as solid black instead of fully fogging.
  // only worth discarding in the End (DimensionID == 2.0) - in the overworld
  // it just eats terrain right at the fog line and looks worse than the bug.
  #ifdef NL_FOG
    if (DimensionID.x == 2.0 && v_fog.a > 0.99) {
      discard;
    }
  #endif

...
void main() {
    ...
    #endif
    #ifdef NL_DEBUG_AURORATEX_TEST
      // TEST ONLY: bypass all masking/math and paint the raw noise texture
      // across the whole dome, to check whether s_AuroraNoiseTex is
      // actually bound to real data or returning black/garbage.
      vec2 dbgUV = vec2(atan2(viewDir.x, viewDir.z)*0.5+0.5, viewDir.y*0.5+0.5);
      vec4 dbgTex = texture2D(s_AuroraNoiseTex, dbgUV);
      gl_FragColor = vec4(dbgTex.rgb, 1.0);
      return;
    #endif
    #ifdef NL_SKY_AURORA_3D
      skyColor += NL_SKY_AURORA*nlRenderVolumetricAurora(viewDir, env, s_AuroraNoiseTex, v_underwaterRainTimeDay.z);

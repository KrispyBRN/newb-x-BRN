...
bool detectSoulSandValley(vec3 FOG_COLOR) {
  // soul sand valley fog (#1b4745) is the only nether biome where green and
  // blue are both well above red and roughly equal to each other - nether
  // wastes/crimson forest are red-dominant, warped forest has low green,
  // basalt deltas is a flat grey. this uniquely isolates it.
  return (FOG_COLOR.g - FOG_COLOR.r > 0.08) && (FOG_COLOR.b - FOG_COLOR.r > 0.08) && abs(FOG_COLOR.g - FOG_COLOR.b) < 0.05;
}

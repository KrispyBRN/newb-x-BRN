...
struct nl_environment {
  ...
  float rainFactor;
  float dayFactor;
  float horizonSign; // sign of sin(timeOfDay*2pi): <0 near sunrise, >0 near sunset
  vec3 sunDir;
  vec3 moonDir;
  vec3 fogCol;
};
...
bool detectSoulSandValley(vec3 FOG_COLOR) {
  // soul sand valley fog is #1b4745 (wiki value) - check how close the
  // live FogColor is to that exact target instead of a relational heuristic
  vec3 target = vec3(0.106, 0.278, 0.271); // #1b4745 normalized
  vec3 d = abs(FOG_COLOR - target);
  return d.r < 0.06 && d.g < 0.06 && d.b < 0.06;
}
...
nl_environment calculateSunParams(nl_environment env, float TIME_OF_DAY, float DAY) {
  ...
  env.horizonSign = sunDir.x; // same sign convention as the SunMoon dawn/dusk split

...
// "3D" aurora - volumetric plane-marched curtain, ported from a shader
// Kevin found. Marches several depth layers along the view ray projected
// onto a horizontal plane, giving real parallax as the layers recede -
// much more convincing depth than the old scrolling-texture version.
// PERF NOTE: each sample does 3 texture2D() reads, so sample count directly
// multiplies fetch cost (NL_SKY_AURORA_3D_SAMPLES=6 -> 18 fetches/pixel in
// the visible band). The galaxy-stars attempt lost ~18fps from just ONE
// noise3D() call, so start low and raise NL_SKY_AURORA_3D_SAMPLES only if
// fps holds up.
vec3 nlRenderVolumetricAurora(vec3 vDir, nl_environment env, sampler2D noiseTex, float t) {
  if (env.underwater) {
    return vec3_splat(0.0);
  }

  float VdotU = clamp(vDir.y, 0.0, 1.0);
  float visibility = sqrt(max(VdotU*4.5 - 0.225, 0.0));
  ...
  vec2 cameraPosM = vec2(t*NL_SKY_AURORA_3D_SPEED, 0.0);

  const int sampleCount = NL_SKY_AURORA_3D_SAMPLES;
  const int sampleCountP = sampleCount + 10;

  float dither = fastRand(vDir.xz);
  float ditherM = dither + 10.0;
  float detailAnimate = t*NL_SKY_AURORA_3D_DETAIL_ANIMATE;

  for (int i = 0; i < sampleCount; i++) {
    float current = (float(i) + ditherM) / float(sampleCountP);
    current *= current;
    vec2 planePos = wpos.xz*(0.8 + current)*10.0 + cameraPosM;
    planePos *= 0.0007;

    float noise = texture2D(noiseTex, planePos).r;
    noise = 1.0 - 0.8*abs(noise - 0.5);
    noise *= noise; noise *= noise; noise *= noise; noise *= noise; // pow2^4, sharpens into thin streaks
    noise *= texture2D(noiseTex, planePos*8.0 + detailAnimate).b;
    noise *= texture2D(noiseTex, planePos - detailAnimate).g;

    float currentM = 1.0 - current;
    float depthFade = currentM*currentM; depthFade *= depthFade;
    aurora += noise*currentM*mix(NL_SKY_AURORA_3D_COLOR_A, NL_SKY_AURORA_3D_COLOR_B, depthFade);
  }

  aurora *= NL_SKY_AURORA_3D_INTENSITY;
  aurora = aurora*visibility/float(sampleCount);
 ...
 - visible only when looking roughly south.

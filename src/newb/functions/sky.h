...
vec2 nlMilkyWayUV(vec3 vdir, float rotationDeg) {
  ...
  float v = 0.5 - 0.5*vdir.y;
  return vec2(u, v);
}

float nlMilkyWayMask(vec3 vdir, nl_environment env, float rotationDeg) {
  if (env.underwater) {
    return 0.0;
  }
  float angle = atan2(vdir.x, vdir.z) - radians(rotationDeg);
  ...
  float halfWidth = radians(NL_MILKYWAY_WIDTH*0.5);
  float edgeMask = 1.0 - smoothstep(halfWidth*0.7, halfWidth, abs(angle));
  float night = 1.0 - smoothstep(-0.1, 0.05, env.dayFactor);
  return edgeMask*night*(1.0-env.rainFactor);
}

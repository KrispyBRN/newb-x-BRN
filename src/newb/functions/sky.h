...
// Aurora - pure sin/cos curtain effect, no noise3D/noise2D at all (much cheaper than the old galaxy)
vec3 nlRenderAurora(vec3 vdir, nl_environment env, float t) {
  if (env.underwater) {
  ...
  vec3 tint = mix(NL_AURORA_COLOR, NL_AURORA_COLOR2, ribbon);
  vec3 aurora = intensity*tint;

  aurora *= mix(1.0, NL_AURORA_DAY_VISIBILITY, env.dayFactor);

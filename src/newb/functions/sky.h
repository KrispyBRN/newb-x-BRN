...
nl_skycolor nlOverworldSkyColors(nl_environment env) {
  ...
  float horizonFactor = 1.0-env.dayFactor*env.dayFactor;
  horizonFactor *= horizonFactor*horizonFactor;
  horizonFactor *= mix(1.0, horizonFactor*horizonFactor, nightFactor);

  // same sign convention as the SunMoon dawn/dusk split: horizonSign<0 near
  // sunrise, >0 near sunset, so each side gets its own color palette
  float dawnFactor = horizonFactor * step(env.horizonSign, 0.0);
  float duskFactor = horizonFactor * step(0.0, env.horizonSign);
  ...
  s.zenith = mix(s.zenith, NL_DUSK_ZENITH_COL, duskFactor);
  s.horizon = mix(s.horizon, NL_DUSK_HORIZON_COL, duskFactor);
  s.horizonEdge = mix(s.horizonEdge, NL_DUSK_EDGE_COL, duskFactor);

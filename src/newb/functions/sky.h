...
// Galaxy stars - optimized to 1 noise3D() call + 2 cheaper noise2D() calls (was 3x noise3D)
vec3 nlRenderGalaxy(vec3 vdir, vec3 fogColor, nl_environment env, float t) {
  ...
  vec3 stars = vec3_splat(0.0);
  ...
  #if defined(NL_GALAXY_STARS_LAYER) || defined(NL_GALAXY_GLOW_LAYER)
    // n1: one real (expensive) 3D lattice noise call, keeps overall structure grounded in 3D space
    float n1 = noise3D(15.0*vdir + sin(0.85*t + 1.3));
    // n2/n3: cheaper 2D noise approximations (4 hash taps vs noise3D's permutation chain)
    // instead of full 3D lattice noise, projected onto different axis pairs for variety
    float n2 = noise2D(50.0*vdir.xz + 1.0*n1 + sin(0.7*t + 1.0));
  #endif

  #ifdef NL_GALAXY_STARS_LAYER
    // stars
    float n3 = noise2D(200.0*vdir.xy - 10.0*sin(0.4*t + 0.500));
    ...
    stars = (0.8 + 0.2*sin(vec3(8.0,6.0,10.0)*(2.0*n1+0.8*n2) + vec3(0.0,0.4,0.82)))*st;
  #endif

  #ifdef NL_GALAXY_GLOW_LAYER
    // glow
  ...
  vec3 auroraTint = mix(NL_GALAXY_COLOR, NL_GALAXY_COLOR2, smoothstep(0.3, 0.7, n0));
  stars *= auroraTint;

...
float nlRenderUnderwaterGodRay(vec3 worldPos, float t, vec3 sunDir) {
  // build a 2D frame perpendicular to the sun direction, so the shaft
  // pattern is actually angled toward where the sun is, instead of being
  // locked to fixed world axes like the overworld NL_GODRAY is
  ...
  float diff = u*0.15 + v*0.05 + t*0.6;

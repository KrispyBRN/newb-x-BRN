...
vec4 nlWater(
  ...
  float waterStormSpeed = mix(NL_WATER_WAVE_SPEED_RAIN, NL_WATER_WAVE_SPEED_THUNDER, smoothstep(0.5, 1.0, env.rainFactor));
  float waterWaveSpeed = mix(NL_WATER_WAVE_SPEED, waterStormSpeed, env.rainFactor);

  vec2 bump = vec2_splat(movingNoise2D(gPos.xz + gPos.yy, waterWaveSpeed*t, 0.6));
  ...
  if (fractCposY > 0.0) { // top plane
    ...
    bump *= 0.5 + 0.5*sin(3.0*t*waterWaveSpeed + cPos.y*PI_HALF);

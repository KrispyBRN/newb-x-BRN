...
void nlWave(
  ...
  if (isColored && !isTreeLeaves && !isLeafLitter && uv0.y>0.214 && uv0.y<0.502 && !isRedStone) {
    ...
    if (shouldWave) {
      ...
      float stormSpeed = mix(
        isTreeLeaves ? NL_LEAF_WAVE_SPEED_RAIN : NL_PLANTS_WAVE_SPEED_RAIN,
        isTreeLeaves ? NL_LEAF_WAVE_SPEED_THUNDER : NL_PLANTS_WAVE_SPEED_THUNDER,
        smoothstep(0.5, 1.0, rainFactor));

      wave *= 1.0 + mix(
        sin(t*NL_WAVE_SPEED + phaseDiff),
        sin(t*stormSpeed + phaseDiff),
        rainFactor);
